...
import {
  EnumLanguage,
  InputContentPurchaseInfo,
  useCreateOrUpdateContentPurchaseInfoMutation,
  useGetContentPurchaseInfoQuery
} from 'src/graphql-definition/webinfo-service.generated'

import { EnumTrangQuanLyPurchaseInfo, ITabStatePurchaseInfo } from './interface'

import './styles.less'

function CMSPurchaseInfo() {
  ...
  const { data, loading, refetch } = useGetContentPurchaseInfoQuery({
  ...
  const [callCreateOrUpdate, { loading: loadingCreateOrUpdate }] =
    useCreateOrUpdateContentPurchaseInfoMutation({
      ...
      onCompleted: async (data) => {
        if (data?.createOrUpdateContentPurchaseInfo?._id) {
  ...
  const [currentTab, setCurrentTab] = useState<ITabStatePurchaseInfo>(
    EnumTrangQuanLyPurchaseInfo.Info
  )
  const handleChangeTab = useCallback(
    (tab: EnumTrangQuanLyPurchaseInfo) => setCurrentTab(tab),
    []
  )
  const listTabs_i18n: Array<{
    code: EnumTrangQuanLyPurchaseInfo | 'SEO Information'
    name: string
  }> = [
    {
      code: EnumTrangQuanLyPurchaseInfo.Info,
      name: t('manageAboutUs.textSEOInfo')
    },
    {
      code: EnumTrangQuanLyPurchaseInfo.Content,
  ...
  const onSubmitCK = () => {
    if (content) {
      const input: InputContentPurchaseInfo = {
        Content: content
      }
  ...
  const setFieldInfoData = () => {
    if (data?.getContentPurchaseInfo) {
      const { SEODescription, SEOKeywords, SEOTitle } =
        data?.getContentPurchaseInfo
  ...
  const setFieldCKData = () => {
    if (data?.getContentPurchaseInfo) {
      setContent(data?.getContentPurchaseInfo?.Content || '')
    }
  }
  ...
  return (
    <div className='cms_page'>
      <div className='cms_title'>{t('managePurchaseInfo.textTitle')}</div>
      <IconTabBar activeKey={currentTab} onTabClick={handleChangeTab}>
        ...
        <IconTabBar.Content>
          ...
          >
            {currentTab === EnumTrangQuanLyPurchaseInfo.Info && (
            ...
            {currentTab === EnumTrangQuanLyPurchaseInfo.Content && (
              // <>
...
export default CMSPurchaseInfo

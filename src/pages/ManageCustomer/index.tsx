...
export default React.memo(() => {
  ...
  const componentDidMount = useCallback(async () => {
    ...
    if (hrefArr.splice(-1).toString() === 'manage-customer') {
      if (dataCategoriesGroup) {
        const firstChildRoute = dataCategoriesGroup
          .slice(0, 1)
          .map((child) => child.route)
          .toString()
        if (firstChildRoute) {
          history.push(`/manage-customer${firstChildRoute}`)
        }
      }
    }
  ...
  const handleSelect = (params: NavigationListEventParam) => {
    ...
    if (finded?.type === 'his') {
      history.push(`/manage-customer${finded.route}`)
    } else {
      window.location.href = `${window.location.origin}/manage-customer${finded?.route}`
    }
  }

  useEffect(() => {
    const prevTitle = window.document.title
    window.document.title = t('subscriber.titleManageCustomer')
      ...
      <CommonHisComponent.SplitContainer
        ...
        renderLeftContainer={() =>
          !!(
            ...
            <div>
              <NavigationList
                title={t('subscriber.titleEcommerceCustomer')}

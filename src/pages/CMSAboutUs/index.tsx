...
import {
  EnumLanguage,
  InputContentAboutUs,
  useCreateOrUpdateContentAboutUsMutation,
  useGetContentAboutUsQuery
} from 'src/graphql-definition/webinfo-service.generated'

import { EnumTrangQuanLyAboutUs, ITabStateAboutUs } from './interface'
...
function CMSAboutUs() {
  ...
  const { data, loading, refetch } = useGetContentAboutUsQuery({
  ...
  const [callCreateOrUpdate, { loading: loadingCreateOrUpdate }] =
    useCreateOrUpdateContentAboutUsMutation({
      ...
      onCompleted: async (data) => {
        if (data?.createOrUpdateContentAboutUs?._id) {
  ...
  const [currentTab, setCurrentTab] = useState<ITabStateAboutUs>(
    EnumTrangQuanLyAboutUs.Info
  )
  const handleChangeTab = useCallback(
    (tab: EnumTrangQuanLyAboutUs) => setCurrentTab(tab),
    []
  )
  ...
  const onSubmitCK = () => {
    if (content) {
      const input: InputContentAboutUs = {
        Content: content
      }
  ...
  const setFieldInfoData = () => {
    if (data?.getContentAboutUs) {
      const { SEODescription, SEOKeywords, SEOTitle } = data?.getContentAboutUs
  ...
  const setFieldCKData = () => {
    if (data?.getContentAboutUs) {
      setContent(data?.getContentAboutUs?.Content || '')
    }
  }
  ...
  return (
    <div className='cms_page'>
      <div className='cms_title'>{t('manageAboutUs.textTitle')}</div>
      <IconTabBar activeKey={currentTab} onTabClick={handleChangeTab}>
        ...
        <IconTabBar.Content>
          ...
          >
            {currentTab === EnumTrangQuanLyAboutUs.Info && (
            ...
            {currentTab === EnumTrangQuanLyAboutUs.Content && (

...
export default React.memo(() => {
  ...
  const componentDidMount = useCallback(async () => {
    ...
    if (arrPathName.splice(-1).toString() === 'manage-page') {
      if (dataPagesGroup) {
        const firstChildRoute = dataPagesGroup
          .slice(0, 1)
          .map((child) => child.route)
          .toString()
        if (firstChildRoute) {
          history.push(`/manage-page${firstChildRoute}`)
        }
      }
    }
  ...
  useEffect(() => {
    ...
    window.document.title = 'Manage page'
      ...
      <CommonHisComponent.SplitContainer
        ...
        renderLeftContainer={() =>
          !!(
            ...
            <NavigationList
              title={t('managePages.titleManagePage')}

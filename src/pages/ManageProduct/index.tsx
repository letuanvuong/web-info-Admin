...
export default React.memo(() => {
  ...
  const componentDidMount = useCallback(async () => {
    ...
    if (hrefArr.splice(-1).toString() === 'manage-product') {
      if (dataCategoriesGroup) {
        const firstChildRoute = dataCategoriesGroup
          ...
          .toString()
        if (firstChildRoute) {
          history.push(`/manage-product${firstChildRoute}`)
        }
      }
    }
  ...
  useEffect(() => {
    ...
    window.document.title = t('product.title')
      ...
      <CommonHisComponent.SplitContainer
        ...
        renderLeftContainer={() =>
          !!(
            ...
            <NavigationList
              title={t('product.titleEcommerceProduct')}
